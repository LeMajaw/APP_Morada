...
export default function NovaMorada(){
   ...
   async function loadMorada(){
      try{
         const response = await api.get(`api/moradas/${moradaId}`,authorization) ;
   ...
   async function saveOrUpdate(event) {
      ...
      try{
         if(moradaId==='0')
         {
            await api.post('api/moradas',data,authorization);
         }
         else
         {
            data.id= id;
            await api.put(`api/moradas/${id}`,data,authorization)
         }
      }catch(error){
